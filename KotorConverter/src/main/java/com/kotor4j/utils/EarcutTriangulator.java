...
/**
 * Triangulator from there: https://github.com/earcut4j/earcut4j
 */
public final class EarcutTriangulator {
     ...
     */
    public static TriangulatedMesh earcut(float[] data) {
        List<Integer> indicies = earcut(data, null, 3);
        return new TriangulatedMesh(data, convert(indicies));
    }

    private static int[] convert(List<Integer> values) {
     ...
     * 12-vertice input would mean one hole with vertices 5–7 and another with
     * 8–11).
     * @param dim is the number of coordinates per vertice in the input array
     * @return List containing groups of three vertice indices in the resulting
     * array forms a triangle.
     */
    private static List<Integer> earcut(float[] data, int[] holeIndices, int dim) {
    ...
    private static void earcutLinked(Node ear, List<Integer> triangles, int dim, float minX, float minY, float size, int pass) {
        if (ear == null) {
            return;
        }
