...
public class TpcDecompress {
    ...
    public void decompress(TpcTexture texture) throws IOException {
        ...
        for (TpcMipMap mipMap : texture.getMipMaps()) {
            NwnByteArrayInputStream src = new NwnByteArrayInputStream(Arrays.copyOf(mipMap.getData(), mipMap.getData().length));
    ...
    private long read48LE(NwnByteArrayInputStream stream) throws IOException {
        long output = stream.readInt() & 0xffffffffl;
        long second = stream.readShort() & 0xffff;
    ...
    private byte[] decompressDxt5(TpcTexture texture, NwnByteArrayInputStream stream) throws IOException {
        ...
        for (int ty = height; ty > 0; ty -= 4) {
            ...
            for (int tx = 0; tx < width; tx += 4) {
                int alpha0 = stream.readByte();
                int alpha1 = stream.readByte();
                long alphabl = read48LE(stream);
                int color0 = stream.readShort() & 0xffff;
                int color1 = stream.readShort() & 0xffff;
                long colors = little2big(stream.readInt()) & 0xffffffffl;
                alphab[0] = alpha0;
                alphab[1] = alpha1;
    ...
    private byte[] decompressDxt1(TpcTexture texture, NwnByteArrayInputStream stream) throws IOException {
        int width = texture.getWidth();
        int height = texture.getHeight();
        int size = width * height * 4;
        int pitch = width * 4;
