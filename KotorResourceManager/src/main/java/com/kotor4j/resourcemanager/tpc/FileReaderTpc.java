 ...
 */
public class FileReaderTpc {

    public TpcTexture loadFile(NwnByteArrayInputStream stream,  String fileName) throws IOException {
        ...
        readData(tpc, stream);
        readTxi(tpc, stream.length(), stream);
        verticalMirror(tpc);
        decompress(tpc);
        convertToRGBA(tpc);
        return tpc;
    ...
    private void readData(TpcTexture tpc, NwnByteArrayInputStream stream) throws IOException {
...
// If the texture width is a power of two, the texture memory layout is "swizzled"
    ...
    private void readTxi(TpcTexture tpc, int size, NwnByteArrayInputStream stream) throws IOException {
        int txiDataSize = (int) (size - stream.getPosition());
        if (txiDataSize == 0) {
            return;
        }
        try {
    ...
    private void readHeader(TpcTexture tpc, NwnByteArrayInputStream stream) throws IOException {
        ...
        int mipMapCount = stream.readByte();
        stream.skip(114);
